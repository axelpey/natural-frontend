...
async def search_books(
    ...
    if title:
        query_result = [book for book in query_result if title.lower() in book["title"].lower()]
        
    if author:
        query_result = [book for book in query_result if author.lower() in book["author"].lower()]
        
    if genre:
        query_result = [book for book in query_result if genre.lower() == book["genre"].lower()]
...
async def update_book(book_id: int, book: Book):
    index = next((index for (index, d) in enumerate(books_db) if d["id"] == book_id), None)
